...
**Контекст: Всё больше пользователей веб-сервиса, над которым вы работаете, пользуются реализованной платформой, и
администраторы начинают замечать проблемы со скоростью. Проведенное нагрузочное тестирование выявило проблемы, с
которыми необходимо разобраться.**
...
- [services.py](catalog/services.py) - реализация сервисных функций
...
1. Приложение [catalog](catalog).
   ...
   Реализация работы интернет-магазина каталога товаров:
    ...
    - [forms.py](catalog/forms.py). Создана форма для модели Product c валидацией полей name, description на проверку
      ...
      формы.
    - [services.py](catalog/services.py). Реализованы функции кэширования данных и фильтрации продуктов по заданной
      категории.
